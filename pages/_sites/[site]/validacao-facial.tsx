...
let stream = await navigator.mediaDevices.getUserMedia(constraints);
    ...
    const onSubmit = async (data) => {
        ...
        } catch (error) { // erro de api e execução 
    ...
    if(router.query["cpf"] === undefined || new CPF().valida(router.query.cpf) == "CPF Inválido") {
        return (
            ...
            <Container minW='calc(100vh)'  minH='calc(100vh)'>
                <Center minW='calc(100vh)'  minH='calc(100vh)' >
                    <Grid templateColumns={{
                        ...
                        <GridItem w='100%'>
                            <Box px={8} py={4} w='100%'h="100%" bg='white' borderRadius='lg'>
...
<Text fontSize='sm'>2º ETAPA - Clique em continuar.</Text>
...
export const getServerSideProps = async (ctx) => {
    ...
    const imobiliaria = await prisma.imobiliaria.findFirst({ where: { url: site }});
